...
def test_notification_type_str():
    ...
    notif_type = NotificationType.objects.create(
        name="Like", slug="like", category="activity", space=space, is_default=True
    )
...
def test_notification_type_unique_slug():
    ...
    NotificationType.objects.create(
        name="Comment", slug="comment", category="activity", space=space
    )
    with pytest.raises(IntegrityError):
        NotificationType.objects.create(
            name="Comment 2", slug="comment", category="activity", space=space
        )
...
def test_user_notification_preference_unique_together():
    ...
    space = Space.objects.create(name="Test Space", slug="test-space")
    notif_type = NotificationType.objects.create(
        name="DM", slug="dm", category="activity", space=space
    )
    UserNotificationPreference.objects.create(
        user=user, notification_type=notif_type, via_email=True, via_platform=True
    )
    with pytest.raises(IntegrityError):
        UserNotificationPreference.objects.create(
            user=user, notification_type=notif_type, via_email=False, via_platform=False
        )
...
def test_user_notification_preference_defaults():
    ...
    space = Space.objects.create(name="Test Space", slug="test-space")
    notif_type = NotificationType.objects.create(
        name="Space Post", slug="space-post", category="space_post", space=space
    )

...
def test_group_belongs_to_community():
    user = User.objects.create_user(email="owner@company.com", password="pass")
    community = models.Community.objects.create(
        name="Test Community", slug="test-community", owner=user
    )
    group = models.Group.objects.create(
        name="Test Group", community=community, position=1
    )
...
def test_space_creation_and_moderators():
    ...
    group = models.Group.objects.create(name="Test Group", community=community)
...
def test_membership_unique_constraint():
    ...
    community = models.Community.objects.create(
        name="Test Community", slug="test-community", owner=owner
    )
    group = models.Group.objects.create(name="Test Group", community=community)
    space = models.Space.objects.create(
        name="Test Space", slug="test-space", group=group, community=community
    )
...
def test_post_and_comment_creation():
    ...
    post = models.Post.objects.create(
        user=user, space=space, title="Test Post", content="Post content"
    )
    comment = models.Comment.objects.create(post=post, user=user, content="A comment")
...
def test_comment_threading():
    ...
    child_comment = models.Comment.objects.create(
        post=post, user=user, content="Child comment", parent=parent_comment
    )
    assert child_comment.parent == parent_comment
    assert models.Comment.objects.filter(parent__id=parent_comment.id).count() == 1
